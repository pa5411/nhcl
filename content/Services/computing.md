+++
title = 'Computer and Printer Facilities'
date = 2026-08-14T07:07:07+01:00
draft = false
showAuthor = false
showDate = true
showWordCount = false
showReadingTime = false
showRelatedContent = false
showPagination = false
+++

## Overview 

{{< lead >}}
Use our computer facilities
{{< /lead >}}

A fast free WiFi connection, computers and a multi printer, scanner and photocopier. We have you covered!

***

## WiFi

WiFi is available free of charge throughout the Library. See inside for details. 

***

## Computers

Our computers are free of charge to use and available on a first come, first served basis for all members of the public. 

> [!IMPORTANT]
> Library membership is not required to use the computers, but we do have a fair usage and access policy. 

{{< feature-grid columns="3" >}}
{{< feature title="Internet Browsing" >}}
Browse the internet at blazing fast speed!
{{< /feature >}}
{{< feature title="Write documents" >}}
Fill out forms and write CVs.
{{< /feature >}}
{{< feature title="Printing" >}}
Print documents, images, forms etc! 
{{< /feature >}}
{{< /feature-grid >}}

Software:
`Microsoft Office`, `Adobe Reader`, `Google Chrome`.

***

## Printing

We offer B&W (black and white) and colour printing onto both A4 and A3 paper for a small fee, depending on the printing job.

### Printing costs

{{< tabs >}}

    {{< tab label="A4 Paper" >}}
        {{< stats columns="4">}}
            {{< stat-nolabel value="20p" >}}Single-sided black & white A4 print.{{< /stat-nolabel >}}
            {{< stat-nolabel value="40p" >}}Double-sided black & white A4 print.{{< /stat-nolabel >}}
            {{< stat-nolabel value="60p" >}}Single-sided colour A4 print.{{< /stat-nolabel >}}
            {{< stat-nolabel value="£1.20" >}}Double-sided colour A4 print.{{< /stat-nolabel >}}
        {{< /stats >}}
    {{< /tab >}}

    {{< tab label="A3 Paper" >}}
        {{< stats columns="4">}}
            {{< stat-nolabel value="£1" >}}Single-sided black & white A3 print.{{< /stat-nolabel >}}
            {{< stat-nolabel value="£2" >}}Double-sided black & white A3 print.{{< /stat-nolabel >}}
            {{< stat-nolabel value="£2" >}}Single-sided colour A3 print.{{< /stat-nolabel >}}
            {{< stat-nolabel value="£4" >}}Double-sided colour A3 print.{{< /stat-nolabel >}}
        {{< /stats >}}
    {{< /tab >}}

{{< /tabs >}}


### How to print

> [!TIP]
> We can provide assistance at any point if needed.

{{< steps >}}
{{< step number="1" title="Log into the computer" >}}Pick a computer and sign in{{< /step >}}
{{< step number="2" title="Open the file" >}}Open the file you wish to print{{< /step >}}
{{< step number="3" title="Send to printer" >}}Press the print button{{< /step >}}
{{< step number="4" title="Check the settings" >}}Select the Samsung printer, make it colour if desired{{< /step >}}
{{< step number="5" title="Talk to a volunteer" >}}A volunteer will print your document at the printer{{< /step >}}
{{< /steps >}}

### Printer FAQs

{{< accordion mode="collapse" >}}
  {{< accordionItem title="Can I print something directly from my phone or laptop?" open=true >}}
  We do not offer WiFi printing. 
  
  However, if you are unable to log into one of our computers to access your documents, you can send your document(s) from your device to our secure email address. We can then print the documents and will delete the email immediately afterwards.  
  {{< /accordionItem >}}

  {{< accordionItem title="can I print something directly from a USB stick?" >}}
  Yes, all of our computers have USB Type A and USB Type C ports.
  {{< /accordionItem >}}
{{< /accordion >}}

***

## Photocopying

### Photocopying costs

This is the same as the printing costs.

### How to photocopy

{{< steps >}}
{{< step number="1" title="Talk to a volunteer" >}}A volunteer will photocopy your document(s){{< /step >}}
{{< /steps >}}

***

## Scanning

### Scanning costs

A flat fee of `20p` for black and white or colour and a reasonable number of documents to scan.

### How to scan

{{< steps >}}
{{< step number="1" title="Talk to a volunteer" >}}A volunteer will scan your document(s){{< /step >}}
{{< step number="2" title="Select a computer or USB stick" >}}We can scan the document(s) directly to one of our computers or to a USB (Type A) stick{{< /step >}}
{{< /steps >}}
